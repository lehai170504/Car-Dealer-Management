...
// Giả định: URL logo của bạn (không sử dụng trong React này)
// const EVM_LOGO_URL = "/path/to/your/evm-logo.svg"; 
...
export default function LoginPage() {
  ...
  // Hàm xử lý đăng nhập giả định
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setIsLoading(true);

    // Dựa vào email để giả định vai trò (trong môi trường thực tế, dùng API)
    let userRole = '';
    if (email.toLowerCase().includes('@evm.com')) {
        userRole = 'EVM';
    } else if (email.toLowerCase().includes('@dealer.com')) {
        userRole = 'DEALER';
    } else {
        userRole = 'EVM'; // Mặc định cho demo
    }
    ...
    setTimeout(() => {
        ...
        if (email.length > 5 && password.length > 5) {
            if (userRole === 'EVM') {
              // Chuyển hướng đến Dashboard của Hãng xe
              router.push('/evm/dashboard');
            } else if (userRole === 'DEALER') {
              // Chuyển hướng đến Dashboard của Đại lý
              router.push('/dealer/dashboard');
            } else {
                setError("Đăng nhập thành công nhưng vai trò không hợp lệ.");
            }
        } else {
  ...
  const TechOverlay = () => (
    <div className="absolute inset-0 overflow-hidden opacity-50">
        <div className="absolute top-0 left-0 w-full h-full bg-no-repeat bg-cover opacity-10" 
             style={{ backgroundImage: 'radial-gradient(circle at 100% 0%, rgba(0,255,140,0.1), transparent 50%)' }}>
        </div>
        {/* Abstract Data Flow Grid */}
        <div className="absolute inset-0 bg-[length:20px_20px] opacity-10" 
             style={{ backgroundImage: 'linear-gradient(to right, #00A89A 1px, transparent 1px), linear-gradient(to bottom, #00A89A 1px, transparent 1px)' }}>
        </div>
    </div>
  );
  ...
  return (
    // Bố cục toàn màn hình (grid 2 cột)
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      
      {/* Cột 1: Branding & Hình ảnh (Sử dụng gradient và hiệu ứng) */}
      <div className="hidden lg:flex flex-col justify-center items-center p-12 
                    bg-gradient-to-br from-gray-900 to-gray-800 text-white relative shadow-2xl">
        ...
        <div className="z-10 text-center space-y-6">
            ...
            <div className="flex items-center justify-center space-x-3 text-4xl font-extrabold tracking-tight">
                {/* Logo VoltLink/Connect - Xanh Electric Green */}
            ...
            <p className="text-xl text-gray-400 mt-4 max-w-lg mx-auto">
                Nền tảng hợp nhất giúp theo dõi hiệu suất, tồn kho và trải nghiệm khách hàng giữa Hãng xe và Đại lý theo thời gian thực.
            </p>

            {/* Minh họa Công nghệ Trừu tượng */}
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="flex justify-end">
                <Link href="#" className="text-sm font-medium text-emerald-600 hover:text-emerald-700 hover:underline transition">
                  Quên mật khẩu?
                </Link>
              </div>
            ...
            <div className="mt-8 text-center text-sm text-gray-500">
              <p>Hệ thống được vận hành bởi EVM. <Link href="#" className="font-medium text-gray-600 hover:underline">Điều khoản dịch vụ.</Link></p>
            </div>
          </CardContent>
        </Card>
      </div>
